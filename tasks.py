...
def coverage(c):
    ...
    result = pytest.main(["-v", "--cov", "micromagneticdata", "--cov-report", "xml"])
    raise Exit(code=result)
...
def docs(c):
    """Run doctests."""
    result = pytest.main(
        ["-v", "--doctest-modules", "--ignore", "micromagneticdata/tests", "micromagneticdata"]
    )
    raise Exit(code=result)
